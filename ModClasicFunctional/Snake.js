class Snake
{
    ...
    {
        if(snake_body[0].x < 0 || snake_body[0].x > canvas.width)
           return true;//alert("ai pierdut .Apasa F5 ca sa dai restart. Scorul tau a fost de "+String(score));
        if(snake_body[0].y < 0 || snake_body[0].y > canvas.height)
           return true;//alert("ai pierdut.Apasa F5 ca sa dai restart. Scorul tau a fost de "+String(score));
        return false;
    }
    coliziune_mancare()
